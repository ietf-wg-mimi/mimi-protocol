...
The franking mechanism requires the message format to contain a random salt,
the sender URI, and the room URI. It describes the exact way to extract these
fields from the MIMI content format.

The MIMI content format {{!I-D.ietf-mimi-content}} includes a per-message,
mandatory, cryptographically random 256-bit salt generated by the sender.
(An example mechanism to safely generate the salt is discussed in Section 8.2 of
