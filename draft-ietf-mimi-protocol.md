...
second is the `franking_agents` GroupContext component {{Section 4.6 of
!I-D.ietf-mls-extensions}}. The structure of `franking_agents` mirrors that of
the `external_senders` extension described in {{Section 12.1.8.1 of !RFC9420}}.
It contains a list of signature key/credential pairs.
...
FrankingAgent franking_agents<V>;
~~~
...
`franking_agents<V>` is also the format of the `update` field inside the
AppDataUpdate struct in an AppDataUpdate Proposal for the `franking_agents`
component. The use of AppDataUpdate on the `franking_agents` is RECOMMENDED only
when adding a new `FrankingAgent`. To avoid confusion about which signature key
to use, when an MLS client rotates the `franking_agents`, the client SHOULD
create a new MLS group by sending a ReInit proposal.
...
When a valid `franking_agents` application component {{iana-franking-sig-key}}
is present in the GroupContext, the Hub can frank messages, and sign the frank
with the Hub's `franking_private_key` corresponding to the
`franking_signature_key` in the FrankingAgent struct for the Hub's domain.
...
MLS cipher suite in-use, the client looks for the `franking_agents`
application component in the GroupContext. It finds one of the `franking_agents`
has an entry with a `credential` that corresponds to the domain of the Hub, and
extracts the corresponding `franking_signature_key`.
...
with the `franking_signature_key` in the `franking_agents` component in the
GroupContext.
