...
performed exclusively with the hub server for a room. Events that transit from
a follower server, through a hub, and onwards onto another follower server are
signed to ensure authenticity over all network hops.
...
struct {
   ...
   select (Event.type) {
      case "m.room.user":
         // Some event types, like m.room.user, require a signature because
         // the event transits over multiple network hops. The signature
         // covers the whole Event struct, and is generated by the origin
